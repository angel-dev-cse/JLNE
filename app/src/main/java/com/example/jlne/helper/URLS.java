package com.example.jlne.helper;

public class URLS {
    public static final String BASE_URL = "http://127.0.0.1:8080/jlne/API.php?action=";
    public static final String LOGIN_URL = BASE_URL + "login";
    public static final String REGISTER_URL = BASE_URL + "register";
    public static final String ADD_ORGANIZATION_URL = BASE_URL + "add_organization";
    public static final String GET_ORGANIZATION_URL = BASE_URL + "get_organization";
    public static final String GET_ALL_ORGANIZATION_URL = BASE_URL + "get_all_organization";
    public static final String ADD_MACHINE_URL = BASE_URL + "add_machine";
    public static final String UPDATE_MACHINE_URL = BASE_URL + "update_machine";
    public static final String DELETE_MACHINE_URL = BASE_URL + "delete_machine";
    public static final String GET_MACHINE_FIELDS_URL = BASE_URL + "get_machine_fields";
    public static final String GET_ORGANIZATION_MACHINE_SUMMARY = BASE_URL + "get_organization_machine_summary";
    public static final String GET_MACHINES = BASE_URL + "get_machines";
    public static final String GET_MACHINE = BASE_URL + "get_machine";
    public static final String GET_TRANSPORTATION_BY_MACHINE = BASE_URL + "get_transportation_by_machine";
}
